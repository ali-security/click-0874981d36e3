...
def test_pager_env_passed_to_popen_as_argv(monkeypatch, pager_env, expected_parts):
    ...
    with patch("subprocess.Popen") as mock_popen:
        mock_popen.return_value.stdin = io.BytesIO()
        mock_popen.return_value.wait.return_value = 0
...
def test_echo_via_pager_missing_pipe_pager(monkeypatch, capsys):
    """A ``PAGER`` naming a command that doesn't exist must not raise out of
    ``echo_via_pager``. Without a shell to report the error, the text is
    printed unpaged instead.
    """
    monkeypatch.setattr(click._termui_impl, "WIN", False)
    monkeypatch.setattr(click._termui_impl, "isatty", lambda x: True)
    monkeypatch.setitem(os.environ, "PAGER", "click-nonexistent-pager")
...
def test_echo_via_pager_missing_tempfile_pager(monkeypatch, capsys):
    """The temp-file pager strategy, used on Windows, must not raise or
    swallow the output when the pager command doesn't exist.
    """
    monkeypatch.setattr(click._termui_impl, "WIN", True)
    monkeypatch.setattr(click._termui_impl, "isatty", lambda x: True)
    monkeypatch.setitem(os.environ, "PAGER", "click-nonexistent-pager")
